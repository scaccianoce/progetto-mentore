...
class SchemaDatabase {
  ...
  /// Carica lo schema dal database e lo converte nel modello dinamico.
  static Future<SchemaDatabase> carica({
    DatabaseRepository? database,
    bool forzaAggiornamento = false,
  }) async {
    final db = database ?? DatabaseRepository();
    final raw = await db.caricaSchema(
      forzaAggiornamento: forzaAggiornamento,
    );
    return SchemaDatabase.fromJson(raw);
  }
...
class ConfigurazionePaginaDinamica {
  ...
  /// Ordine opzionale dei campi nella maschera di modifica.
  ///
  /// I campi non elencati mantengono l'ordine restituito dallo schema DB
  /// e vengono collocati dopo quelli esplicitamente indicati.
  /// L'ordine di visualizzazione della pagina resta invece responsabilita'
  /// del relativo `*_page.dart`.
  final List<String> ordineCampi;
...
/// Personalizzazioni della maschera di modifica non appartenenti allo schema PostgreSQL.
///
...
/// per scelte specifiche dell'editor dinamico.
