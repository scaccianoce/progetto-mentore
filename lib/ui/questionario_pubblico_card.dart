...
class QuestionarioPubblicoCard extends StatefulWidget {
  const QuestionarioPubblicoCard({
    super.key,
  ...
  final QuestionarioPubblicoController controller;
  ...
  @override
  State<QuestionarioPubblicoCard> createState() =>
      _QuestionarioPubblicoCardState();
}

class _QuestionarioPubblicoCardState
    extends State<QuestionarioPubblicoCard> {
  QuestionarioPubblicoController get controller => widget.controller;

  @override
  void initState() {
    super.initState();
    controller.addListener(_ridisegna);
  }

  @override
  void didUpdateWidget(covariant QuestionarioPubblicoCard oldWidget) {
    super.didUpdateWidget(oldWidget);
    if (oldWidget.controller == controller) return;
    oldWidget.controller.removeListener(_ridisegna);
    controller.addListener(_ridisegna);
  }
  ...
  Widget build(BuildContext context) {
    final esistente = controller.questionarioPerMentoraggio(widget.mentoraggioId);
    final templateStudenti = controller.templatePer('studenti');
    final url = esistente == null
        ? null
        : controller.urlQuestionario(esistente);
          ...
          children: [
            ...
            if (esistente != null) ...[
              ...
              if (widget.consentiRigenerazione)
                FutureBuilder<int>(
                  future: controller.contaCompilazioni(
                    esistente['id'].toString(),
                  ),
  ...
  Future<void> _genera(List<Map<String, dynamic>> templateStudenti) async {
    ...
    final errore = await controller.creaPerMentoraggio(
  ...
  Future<void> _eliminaPerRigenerare(Map<String, dynamic> questionario) async {
    final conferma =
        ...
        false;
    if (!conferma) return;
    final errore = await controller.elimina(
      questionario['id'].toString(),
    );
    if (!mounted) return;
    _messaggio(
      context,
      errore ?? 'Questionario eliminato. Ora può essere rigenerato.',
    );
    if (errore == null) setState(() {});
  }
  ...
  Future<void> _mostraDomande(
    ...
    final domande = controller.domandeTemplate(
