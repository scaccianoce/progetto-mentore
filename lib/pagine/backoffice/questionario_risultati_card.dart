...
class QuestionarioRisultatiCard extends StatefulWidget {
  ...
  @override
  State<QuestionarioRisultatiCard> createState() => _QuestionarioRisultatiCardState();
}
...
class _QuestionarioRisultatiCardState extends State<QuestionarioRisultatiCard> {
  ...
  Future<void> _carica() async {
    ...
    try {
      final risultato = await widget.controller
          .caricaRisultatiQuestionario(widget.questionario['id'].toString());
      if (!mounted) return;
      setState(() {
        dati = risultato;
        caricamento = false;
        ...
        child: ExpansionTile(
          ...
          children: [
            ...
            if (dati != null) ...[
              ...
              for (final riga in (dati!['sintesi'] as List).cast<Map<String, dynamic>>())
                _sintesi(riga),
              if (widget.consentiExport) ...[
                ...
                Align(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      final errore = await widget.controller
                          .esportaCsvRisultati(widget.questionario['id'].toString());
  ...
  Widget _sintesi(Map<String, dynamic> riga) {
    ...
    if (tipo == 'scala') {
      ...
      dettaglio = frequenze.entries.map((e) => '${e.key}: ${e.value}').join(' · ');
