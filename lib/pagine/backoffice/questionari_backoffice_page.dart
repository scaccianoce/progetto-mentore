...
class _QuestionariBackofficePageState
    extends State<QuestionariBackofficePage> {
  ...
  Future<void> _editorTemplate(
    ...
    var destinatario =
        iniziali?['destinatario']?.toString() ?? 'partecipanti';
  ...
  Future<void> _nuovaDomanda(
    ...
    if (tipo == 'scelta_singola') {
      ...
      final minimo =
          parti.isNotEmpty ? int.tryParse(parti.first.trim()) ?? 1 : 1;
      final massimo =
          parti.length > 1 ? int.tryParse(parti[1].trim()) ?? 5 : 5;
      opzioniJson = <String, dynamic>{
        'min': minimo,
        'max': massimo,
      };
    }
  ...
  Future<void> _eliminaDomanda(
    ...
    final errore =
        await controller.eliminaDomanda(domanda['id'].toString());
