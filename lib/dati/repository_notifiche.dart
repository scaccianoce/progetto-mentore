...
class NotificheRepository {
  NotificheRepository({DatabaseRepository? database, SupabaseClient? client})
  ...
  Future<List<Map<String, dynamic>>> messaggi({int limite = 200}) => _database
      ...
      .elenco(
        ...
        ordinamenti: const <OrdineDb>[OrdineDb('created_at', crescente: false)],
        limite: limite,
      );
  ...
  Future<List<Map<String, dynamic>>> destinatariMessaggio(
    ...
    final destinatari = await _database
        .tabella('notifiche_destinatari')
        .elenco(
          colonne:
          ...
          filtri: <FiltroDb>[FiltroDb.uguale('messaggio_id', messaggioId)],
  ...
  Future<void> aggiornaMessaggio(
    ...
    await _database
        ...
        .aggiorna(
          valori,
          filtri: <FiltroDb>[FiltroDb.uguale('id', messaggioId)],
          colonne: 'id',
        );
  }

  Future<void> eliminaMessaggio(String messaggioId) => _database
      .tabella('notifiche_messaggi')
      .elimina(filtri: <FiltroDb>[FiltroDb.uguale('id', messaggioId)]);

  Future<void> eliminaDestinatari(String messaggioId) => _database
      .tabella('notifiche_destinatari')
      .elimina(
        filtri: <FiltroDb>[FiltroDb.uguale('messaggio_id', messaggioId)],
      );
  ...
  Future<int> generaDestinatari(String messaggioId) async {
    final raw = await _database
        .rpc(
          ...
          parametri: <String, dynamic>{'p_messaggio_id': messaggioId},
        )
  ...
  Future<void> inviaMessaggio(String messaggioId) async {
    try {
      ...
      try {
        await aggiornaMessaggio(messaggioId, <String, dynamic>{
          'stato': 'errore',
        });
      } catch (_) {
  ...
  Future<List<Map<String, dynamic>>> notificheRicevute({
    ...
    return _database
        .tabella('notifiche_destinatari')
        .elenco(
          colonne: 'id, messaggio_id, stato, inviato_at, letto_at, created_at',
          filtri: <FiltroDb>[
            FiltroDb.uguale('user_id', userId),
            const FiltroDb.inLista('stato', <Object>['inviato', 'letto']),
          ],
          ordinamenti: const <OrdineDb>[
            OrdineDb('created_at', crescente: false),
          ],
  ...
  Future<List<Map<String, dynamic>>> messaggiPerIds(List<String> ids) async {
    ...
    return _database
        ...
        .elenco(
          ...
          filtri: <FiltroDb>[FiltroDb.inLista('id', ids.cast<Object>())],
        );
  }
  ...
  Future<void> segnaDestinatarioLetto(String destinatarioId) async {
    await _database.rpc(
      ...
      parametri: <String, dynamic>{'p_destinatario_id': destinatarioId},
    );
  }

  Future<List<Map<String, dynamic>>> regole() => _database
      .tabella('notifiche_regole')
      .elenco(ordinamenti: const <OrdineDb>[OrdineDb('codice')]);
  ...
  Future<void> eliminaRegola(Object id) => _database
      .tabella('notifiche_regole')
      .elimina(filtri: <FiltroDb>[FiltroDb.uguale('id', id)]);
